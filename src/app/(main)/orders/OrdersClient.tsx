        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="w-[20%] text-right">סכום כולל</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <TableRow key={order.id} onClick={() => setSelectedOrder(order)} className="border-border hover:bg-muted/20 transition-colors cursor-pointer group">
                  ...
                  <TableCell className="text-right font-mono font-bold">₪{Number(order.totalAmount).toFixed(2)}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
